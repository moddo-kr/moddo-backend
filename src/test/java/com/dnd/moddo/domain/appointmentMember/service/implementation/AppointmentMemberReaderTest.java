...
public class AppointmentMemberReaderTest {
	...
	void findAllBySettlementIdSuccess() {
		...
		when(appointmentMemberRepository.findBySettlementId(eq(groupId))).thenReturn(expectedMembers);

		//when
		List<AppointmentMember> result = appointmentMemberReader.findAllBySettlementId(groupId);
		...
		verify(appointmentMemberRepository, times(1)).findBySettlementId(groupId);
	}
	...
	void findByGroupMemberIdFail() {
		...
		doThrow(new AppointmentMemberNotFoundException(appointmentMember)).when(appointmentMemberRepository)
			.getById(eq(appointmentMember));

		//when & then
		assertThatThrownBy(() -> {
			appointmentMemberReader.findByAppointmentMemberId(appointmentMember);
